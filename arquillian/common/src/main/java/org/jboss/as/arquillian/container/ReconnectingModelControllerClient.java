 ...
 *
 */
public class ReconnectingModelControllerClient extends AbstractModelControllerClient {
    ...
    ReconnectingModelControllerClient(CallbackHandler handler, String hostName, int connectionTimeout,
            int port, SSLContext sslContext) throws IOException {
    ...
    public ModelNode execute(ModelNode operation, boolean awaitClose) throws IOException {
        ...
        //try {
            response = super.execute(operation);
/*        } catch(IOException e) {
            StreamUtils.safeClose(strategy);
            strategy = null;
            throw e;
...
*/
            ...
            synchronized(lock) {
                if(strategy != null) {
                    ...
                    if(System.currentTimeMillis() - start >= 10000) {
                        new Exception("Haven't been notified").printStackTrace();
                    }
                    StreamUtils.safeClose(strategy);
                    strategy = null;
                }
    ...
    public void ensureConnected(long timeoutMillis) throws IOException {
        boolean doTry = true;
        ...
        while (doTry) {
            ...
            if (ioe != null) {
                if (System.currentTimeMillis() - start > timeoutMillis) {
                    throw new IOException("Failed to establish connection in " + (System.currentTimeMillis() - start)
                            + "ms with " + retries + " retries.", ioe);
                }
                ioe = null;
                try {
                    Thread.sleep(500);
                } catch (InterruptedException e) {
                    throw new IOException("Interrupted while pausing before reconnecting.", e);
                }
            }
            ++retries;
        }
    ...
    static boolean isSuccess(ModelNode operationResponse) {
    ...
    private final class ChannelCloseHandler implements CloseHandler<Channel> {
        ...
        public void handleClose(final Channel closed, final IOException exception) {
            if(ReconnectingModelControllerClient.this.closed) {
                return;
            }
            synchronized(lock) {
                if (strategy != null) {
